...
class GeographicView extends PureComponent {
  ...
  componentDidUpdate(preprops, prestate) {
    ...
    if (!prestate.province.length && province.length && checkUpdate) {
      // 已获取详细地址
      if (this.state.isValue) this.setAddressKeys();
      else this.setState({ allDate: true });
    }
    if (!preprops.value && value && checkUpdate) {
      // 已请求完整省市区
      if (this.state.allDate) this.setAddressKeys();
      else this.setState({ isValue: true });
    }
  }
  ...
  setAddressKeys() {
    ...
    const province_key = province.find(
      x => x.fullname === value.province.label
    );
    ...
    dispatch({
      ...
      payload: {
        id: province_key.id,
        data: { key: province_key.id, label: province_key.fullname }
      }
    }).then(res => {
      this.setState({
        city: res.result[0]
