...
class areaView extends PureComponent {
  ...
  componentDidMount = () => {
    ...
    let geolocation = new qq.maps.Geolocation('DOTBZ-NKEWF-MDHJS-JARVI-6NZI6-ASBKI', 'luotuostore')
    const that = this
    if(!edit) {
  ...
  init = (e) => {
    ...
    let map = new qq.maps.Map(document.getElementById("container"), { zoom: 12 });
  ...
  render() {
    return (
      <Fragment>
        <Button id="clearBtn" style={{marginBottom:10}}>重置</Button>
