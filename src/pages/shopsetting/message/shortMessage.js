...
class ShortMessage extends Component {
  ...
  handleSwitch = item => {
    ...
    Modal.confirm({
      ...
      content: (
        <Fragment>
          确定
          <span style={{ color: "red" }}>{item.switch ? "关闭" : "开启"} </span>
          {item.label} 短信服务
        </Fragment>
      ),
      onOk() {
  ...
  addAmount = () => {
    ...
    const { dispatch } = this.props;
    const self = this;
    if (!amount) return message.error("请输入充值金额");
    dispatch({
    ...
    return (
      <PageHeaderWrapper>
        <Spin spinning={Loading}>
          <Card bordered={false} title={"短信服务开关"}>
            <List
              bordered
          ...
          <Card style={{ marginTop: 30 }} bordered={false} title={"充值"}>
            <Form.Item>
              短信服务通知剩余：
              {balance} 条
            </Form.Item>
            ...
            >
              ...
              <p>1、短信发送收费￥0.1/条</p>
              <p>
                2、充值逢百送十，如充值￥100，实得￥110（即1100条短信），充值￥270，实得￥290（即2900条短信），以此类推。
              </p>
          ...
          <Modal
            ...
            title={
              <div style={{ lineHeight: "28px", display: "inline-flex" }}>
                <Icon
            ...
            footer={[
              <Button onClick={this.initBalance} type={"primary"}>
                确定
              </Button>,
          ...
          >
            <img
              src={
                qrcode.base64_qr_cde &&
                "data:image/png;base64," + qrcode.base64_qr_cde
              }
            />
            <Alert
              type="info"
              showIcon
              message="请用微信扫一扫支付，支付成功后点击“确定”"
            />
          </Modal>
        </Spin>
      </PageHeaderWrapper>
