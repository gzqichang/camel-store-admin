...
class Qualist extends Component {
  ...
  columns = [
    {
      ...
      render: (t, r) => (
        ...
        <span onClick={() => this.delModal(r.id,r.title)}>
         ...
         <span>
          <Link to={{pathname:`/setting/quacontent/editqua`,query: { id:r.id }}}>编辑</Link>
          </span>
        </Fragment>
     )
  }];
  ...
  initData = (type) => {
    const { dispatch, searchform:{ qua } } =this.props
  ...
  render(){
    ...
    return(
      ...
      <Card className={styles.main}>
        <Link to={{pathname:`/setting/quacontent/editqua`,query: { id:null }}}>
          <Button type="primary" style={{marginBottom:20}}>新增内容</Button>
        </Link>
        <Table
          loading={quaLoading}
          dataSource={qualist}
