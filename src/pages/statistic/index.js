...
import { Card, Row, Col, Spin, Table, DatePicker } from "antd";
...
class statistic extends Component {
  ...
  DispatchData = (type, keylist) => {
    ...
    const valuedate =
      type === "order" || type === "turnovers"
        ? orderdate
        : type === "wxuser" || type === "level"
          ? userdate
          : type === "feedback"
            ? feedbackdate
            : type === "withdraw"
    ...
    dispatch({
      ...
      if (res) {
        ...
        keylist &&
        (type === "order" || type === "turnovers" || type === "level")
          ? this.initChartPic(`${type}Pic`, res, keylist.slice(0, -1))
          : null;
      }
    });
  };
  ...
  initChart = (idname, res, keylist) => {
    ...
    const textlist = {
      ...
      turnovers: {
        title: "销售额",
        label: ["普通商品销售额", "订阅商品销售额", "线下销售额", "总销售额"],
      ...
      keylist.map((item, index) => {
        if (item) {
          ...
          _series[index] = {
            ...
            type:
              (textlist[idname].showtype && textlist[idname].showtype[index]) ||
              "line",
            stack:
              (textlist[idname].stack && textlist[idname].stack[index]) ||
              undefined,
  ...
  initChartPic = (idname, res, keylist) => {
    ...
    idname !== "levelPic" &&
      ...
      keylist.map((item, index) => {
        let value = 0;
        res.map(item_in => {
          value = (Number(value) + Number(item_in[item])).toFixed(
            textlist[idname].fixed || 0
          );
        });
    ...
    idname === "levelPic" &&
      res.map(item => {
        data.push({
          value: item.num,
          name: item.level !== "null" ? item.level : "普通会员(无等级)"
        });
      });
  ...
  timeChange = async (value, key) => {
    ...
    if (
      ...
      if (key === "orderdate") {
        ...
        await this.DispatchData("order", [
          "ord_num",
          "sub_num",
          "repl_num",
          "total"
        ]);
        await this.DispatchData("turnovers", [
          "ord_turnovers",
          "sub_turnovers",
          "turnovers"
        ]);
            ...
            <Row gutter={16}>
              ...
              >
                <RangePicker
                  ...
                  onChange={(e, str) => this.timeChange(str, "orderdate")}
                />
              </Col>
              <Col {...colSpan}>
                <div
          ...
          <Spin spinning={loadingdash} tip="数据加载中">
            <Row gutter={16}>
              ...
              >
                <RangePicker
                  style={{ textAlign: "center" }}
                  onChange={(e, str) => this.timeChange(str, "userdate")}
                />
              </Col>
              <Col {...colSpan}>
          ...
          <Spin spinning={loadingdash} tip="数据加载中">
            <Row gutter={16}>
              ...
              >
                <RangePicker
                  style={{ textAlign: "center" }}
                  onChange={(e, str) => this.timeChange(str, "feedbackdate")}
                />
              </Col>
              <Col {...colSpan}>
          ...
          <Spin spinning={loadingdash} tip="数据加载中">
            <Row gutter={16}>
              ...
              >
                <RangePicker
                  style={{ textAlign: "center" }}
                  onChange={(e, str) => this.timeChange(str, "rechargedate")}
                />
              </Col>
            </Row>
        ...
        >
          <Row gutter={16}>
            ...
            >
              <RangePicker
                style={{ textAlign: "center" }}
                onChange={(e, str) => this.timeChange(str, "withdrawdate")}
              />
            </Col>
          </Row>
          <Spin spinning={loadingdash} tip="数据加载中">
