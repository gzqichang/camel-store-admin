...
class Orderlist extends Component {
  ...
  columns = [
    {
      ...
      render: (t, r) =>(
        <Fragment>
          {r.status === 0 ?
            <Fragment>
            <a onClick={() => this.statusChange(r.user_info.nickname,r.id,1)}>已提现</a>
            <Divider type="vertical"/>
            <a style={{color:'red'}}
                onClick={() => this.statusChange(r.user_info.nickname,r.id,2)}>提现失败</a>
            </Fragment>
            : (r.status === 2 ?
                <a style={{color:'red'}} onClick={() => this.remarkModal(r.remark)}>失败原因</a>
                : null
              )
          }
          {r.operation_log &&
  ...
  initData = (type) => {
    const { dispatch, searchform:{ money } } = this.props;
  ...
  render(){
    const { withdrawallist, withdrawallistCount, withdrawalLoading } = this.props
    ...
    return(
      ...
      <Card className={styles.main}>
              ...
              <Fragment>
                <div className={styles.searchitem}>
                  ...
                  <Input
                    ...
                    onChange={(e) => {this.handletradeform(e,'wx_code')} }
                    placeholder="请输入微信号"
                    style={{ width: 200 }}/>
                </div>
                <div className={styles.searchitem}>
                  ...
                  <Input
                    ...
                    onChange={(e) => {this.handletradeform(e,'wxuser')} }
                    placeholder="请输入用户名"
                    style={{ width: 200 }}/>
                </div>
              </Fragment>}
              ...
              <Fragment>
                <div className={styles.searchitem}>
                  ...
                  <Select
                    ...
                    onChange={(e) => {this.handletradeform({target:{value:e}},'status')} }
                    style={{ width: 200 }}
                  >
                ...
                <div className={styles.searchitem}>
                  ...
                  <RangePicker
                    value={tradeform.date_after ? [moment(tradeform.date_after, 'YYYY-MM-DD'), moment(tradeform.date_before, 'YYYY-MM-DD')] : null}
        ...
        <Modal
          ...
          onOk={() => !this.state.loading && this.setState({loading: true}, () => this.handleOk())}
          onCancel={() => this.setState({visible:false})}
        >
          <Input placeholder='请输入提现失败原因'
            value={remark}
