...
class Feedback extends Component {
  ...
  columns = [
      ...
      render: (t, r) => (
        <Fragment>
          <Link
            ...
            to={{ pathname: `/user/userlist/edituser`, query: { id: r.user } }}
          >
    ...
    },
    {
      ...
      render: (t, r) => (
        <span
          onClick={() => this.handleModal(r.update_status, t)}
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          ...
          payload: { url, data: { solve: !solve ? "true" : "false" } }
        }).then(res => {
          if (res) {
            that.initData();
          } else {
    ...
    Modal.confirm({
            ...
            <TabPane tab="反馈情况" key="1">
              <div>
                ...
                {record.user_info && (
                  <span
                    style={{ cursor: "pointer" }}
                    onClick={() => this.toUserDeatal(record.user)}
                  >
                    <img
                      src={record.user_info.avatar_url}
                      width="30"
                      height="30"
                      alt=""
                    />
              ...
              <div style={{ margin: "10px 0" }}>
                反馈时间：
                {moment(record.add_time).format("YYYY-MM-DD kk:mm:ss")}
              </div>
              <div style={{ margin: "15px 0" }}>
                反馈
                {type && type === "goods" ? "商品" : "订单"}：
                {type ? (
                  <a
                    onClick={() =>
                      this.toGoodpage(
                        record.goods || record.order,
                        (record.goods && record.goods_info) ||
                          (record.order && record.order_info),
                        type
                      )
                    }
                  >
              ...
              <div
                style={{ overflowY: "auto", maxHeight: "calc(30vh)" }}
      ...
      onOk() {
        record.update_status
          ? that.handleModal(record.update_status, record.solve)
          : null;
      }
    });
  };
  ...
  toGoodpage = (id, info, type) => {
    const orderType = {
      ord: {
        detail: "/order/orderlist/editorder",
        gooddetail: "/good/ordgood/goodlist/editgood"
      },
      sub: {
        detail: "/order/subscribeOrderlist/subscribeEditorder",
        gooddetail: "/good/subgood/subscribegood/subscribeEdit"
      },
      replace: {
        detail: "/order/replOrderlist/editreplorder",
        gooddetail: "/good/replacegood/replacegoodlist/editreplacegood"
      }
    };
    const pathname =
      type === "goods"
        ? orderType[info.model_type].gooddetail
        : orderType[info.model_type].detail;
                ...
                <Fragment>
                  <div className={styles.searchitem}>
                    ...
                    <Input
                      ...
                      onChange={e => {
                        this.handletradeform(e.target.value, "nickname");
                      }}
                      placeholder="请输入用户名"
                      style={{ width: 200 }}
                    />
                  ...
                  <div className={styles.searchitem}>
                    ...
                    <Input
                      ...
                      onChange={e => {
                        this.handletradeform(e.target.value, "phone");
                      }}
                      placeholder="请输入电话号码"
                      style={{ width: 200 }}
                    />
              ...
              renderAdvancedForm={() => (
                <Fragment>
                  ...
                  <div className={styles.searchitem}>
                    ...
                    <RangePicker
                      value={
                        tradeform.add_time_after
                          ? [
                              moment(tradeform.add_time_after, "YYYY-MM-DD"),
                              moment(tradeform.add_time_before, "YYYY-MM-DD")
                            ]
                          : null
                      }
