...
class EditUser extends Component {
  ...
  changeColumn = type => {
    return [
      ...
      },
      {
        ...
        render: (t, r) =>
          ((r.detail || r.number) && (
            <a onClick={() => this.toDetailPage(r)}>查看详情</a>
          )) ||
          "/"
      }
    ];
  ...
  toDetailPage = (record, type) => {
    ...
    const orderType = {
      ord: {
        detail: is_pt
          ? "/order/mixOrderlist/orderDetail"
          : "/order/orderlist/editorder",
        list: "/order/orderlist",
        searchkey: { order: { user: record.nickname, user_id: record.id } }
      },
      sub: {
        detail: is_pt
          ? "/order/mixOrderlist/suborderDetail"
          : "/order/subscribeOrderlist/subscribeEditorder",
    ...
    if (!type) {
      let pathname = "",
        searchkey = {};
      switch (
        (record.detail && record.detail.model_type) ||
          record.a_type ||
          record.log_type
      ) {
        case "recharge":
        ...
        case "use":
          ...
          searchkey = { outorder: { order_sn: record.number } };
          break; //线下支付
        case "qrpay":
          pathname = "/offline/orders";
          searchkey = { outorder: { order_sn: record.number } };
        ...
        default:
          pathname =
            (record.detail && orderType[record.detail.model_type].detail) ||
            null;
          break;
      }
      ...
      if (record.log_type !== "grouping") {
        ...
        record.detail &&
          ...
          this.props.history.push({
            pathname,
            query: { id: record.detail.link.split("/").reverse()[1] }
          });
      } else {
        dispatch({
          ...
          if (res && res[0]) {
            this.props.history.push({
              pathname: "/order/mixOrderlist/mixEditorder",
              query: { id: res[0].id }
            });
          }
        });
    ...
    if (type) {
      ...
      orderType[type].searchkey
        ? dispatch({
            ...
            this.props.history.push({ pathname: orderType[type].list })
          )
        : this.props.history.push({
            pathname: orderType[type].detail,
            query: { id: record.id }
          });
    }
  };
  ...
  //会员钱包和积分
  handlePrice = (action, key) =>
    this.setState({ visible: true, action, key, amount: 0 });
  okModal = () => {
                ...
                <Form className={styles.editform}>
                  ...
                  <FormItem label="团队" {...formItemLayout}>
                    ...
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => this.toDetailPage(userdata, "teamuser")}
                    >
                      查看
                    </Button>
                  </FormItem>
                  ...
                  <FormItem label="会员钱包余额" {...formItemLayout}>
                    <span>
                      ￥{(userdata.asset + userdata.recharge).toFixed(2) || 0}
                    </span>
                    <Button
                      type="primary"
                      size="small"
                      style={{ marginLeft: 10, marginRight: 10 }}
                      onClick={() => this.handlePrice("add", "wallet")}
                    >
                      赠送
                    </Button>
                    <Button
                      type="danger"
                      size="small"
                      onClick={() => this.handlePrice("subtract", "wallet")}
                    >
                      扣减
                    </Button>
                  </FormItem>
                  ...
                  <FormItem label="积分" {...formItemLayout}>
                    ...
                    <Button
                      ...
                      size="small"
                      style={{ marginLeft: 10, marginRight: 10 }}
                      onClick={() => this.handlePrice("add", "credit")}
                    >
                      赠送
                    </Button>
                    <Button
                      type="danger"
                      size="small"
                      onClick={() => this.handlePrice("subtract", "credit")}
                    >
                      扣减
                    </Button>
                  </FormItem>
                  ...
                  <FormItem label="普通商品订单" {...formItemLayout}>
                    <span>
                      {userdata.order_count &&
                        userdata.order_count.ord_order_count}{" "}
                      个
                    </span>
                    <Button
                      size="small"
                      type="primary"
                      onClick={() => this.toDetailPage(userdata, "ord")}
                    >
                      查看
                    </Button>
                    <span style={{ marginLeft: 40 }}>订阅商品订单：</span>
                    {userdata.order_count &&
                      userdata.order_count.sub_order_count}{" "}
                    个
                    <span>
                      <Button
                        size="small"
                        type="primary"
                        onClick={() => this.toDetailPage(userdata, "sub")}
                      >
                        查看
                      </Button>
                    </span>
                    ...
                    <FormItem label="商品推广返利" {...formItemLayout}>
                      <span>
                        <Switch
                          ...
                          onChange={e =>
                            this.handleModalUser(e, "rebate_right")
                          }
                        />
                      </span>
                    </FormItem>
          ...
          >
            <Fragment>
              ...
              <InputNumber
                ...
                parser={value =>
                  key === "wallet" ? value : value.replace(/\$\s?|(,*)/g, "")
                }
                step={1}
