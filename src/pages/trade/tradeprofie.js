...
class tradeProfie extends Component {
  ...
  columns = [
    {
      ...
      align: "left",
      render: t => (
        <Link to={{ pathname: "/user/userlist/edituser", query: { id: t.id } }}>
          <img src={t.avatar_url} width="30" />
          <span style={{ marginLeft: 10, color: "rgba(0,0,0,0.56)" }}>
            {t.nickname}
          </span>
        </Link>
      )
    ...
    },
    {
      ...
      render: (t, r) => (
        <Fragment>
          {(r.referral_avatar_url && (
            <img
              src={r.referral_avatar_url}
              width="30"
  ...
  setid = record => {
    ...
    if (record.detail) {
      ...
      this.props.history.push({
        pathname: path,
        query: {
          id: record.detail.link && record.detail.link.split("/").reverse()[1]
        }
      });
    }
  };
  ...
  componentDidMount() {
    ...
    const key =
      (location.pathname.includes("distribution") && "profile") ||
    ...
    const textType = [
      {
        type: "accountlog",
        searchkey: { ...profile },
        path: location.pathname.includes("distribution")
      },
      {
        type: "creditlog",
        searchkey: { ...credit, log_type: "share" },
        path: location.pathname.includes("share")
      }
    ];
    const result = textType.filter(item => item.path);
    const { accountform } = this.state;
    let _accountform = {
      log_type: result[0].type === "creditlog" ? "share" : undefined
    };
    if (type === "init") {
      ...
      this.setState({
        accountform:
          result[0].type === "creditlog" ? { log_type: "share" } : {},
        date: null
      });
    } else if (type === "search") {
      _accountform = {
  ...
  componentWillUnmount() {
    ...
    const { dispatch, location } = this.props;
    const searchfrom =
      (location.pathname.includes("distribution") && {
        profile: { ...accountform }
      }) ||
  ...
  render() {
    ...
    const type =
      (location.pathname.includes("distribution") && "accountlog") ||
                ...
                <Fragment>
                  <div className={styles.searchitem}>
                    <span className={styles.searchitem_span}>
                      {type === "accountlog" ? "消费用户:" : "返利用户:"}
                    </span>
                    <Input
                      value={accountform.user}
                      onChange={e => {
                        this.handleAccount(e, "user");
                      }}
                      placeholder={
                        type === "accountlog" ? "请输入消费用户" : "请输入用户"
                      }
                      style={{ width: 200 }}
                    />
                  </div>
                  ...
                  <div className={styles.searchitem}>
                    ...
                    <RangePicker
                      value={
                        accountform.date_after
                          ? [
                              moment(accountform.date_after, "YYYY-MM-DD"),
                              moment(accountform.date_before, "YYYY-MM-DD")
                            ]
                          : null
                      }
