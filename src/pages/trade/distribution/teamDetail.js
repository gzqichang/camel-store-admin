...
class EditUser extends Component {
  ...
  columns = [
      ...
      render: (t,r) => (
        <Fragment>
          <Link style={{color:"rgba(0, 0, 0, 0.65)"}} to={{ pathname: `/user/userlist/edituser`, query:{id: r.id }} }>
            ...
            <span style={{display:'inline-block',verticalAlign:'middle',maxWidth:'100px',
    ...
    },
    {
      ...
      render: (t, r) => (
        <Fragment>
          <Link to={{ pathname: `/trade/distribution/teamlist/teamdetail`, query:{id: r.url.split('/').reverse()[1] }} }>
            <span>查看</span>
          </Link>
        </Fragment>
      ),
  ...
  initData = (type) => {
    ...
    dispatch({
      ...
      this.setState({
        userdata:res,
        userform: { bonus_right: res.bonus_right, rebate_right: res.rebate_right, testers: res.testers }
      })
      if(type === 'init'){
    ...
    return(
      <PageHeaderWrapper>
        ...
        <Card>
          <Card type="innner" title="用户基本信息">
            <Form>
              <FormItem label="用户信息" {...formItemLayout}>
                <span>
                  <Link style={{marginLeft: 10,color:"rgba(0, 0, 0, 0.65)" }} to={{ pathname: `/user/userlist/edituser`, query:{id: userdata.id }} }>
                ...
                {userdata.referrer && userdata.referrer.id &&
                  <span>
                    <Link style={{marginLeft: 10,color:"rgba(0, 0, 0, 0.65)" }} to={{ pathname: `/user/userlist/edituser`, query:{id: userdata.referrer.id }} }>
                      ...
                      <span style={{marginLeft: 10, marginRight: 20 }}>{userdata.referrer.nickname}</span>
                    </Link>
          ...
          <Card type="innner" title={`队员列表 总共${relList.length}人`} style={{margin: '15px 0'}}>
