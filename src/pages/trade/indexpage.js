...
class indexPage extends Component {
      ...
      onOk() {
        dispatch({
          ...
          .then(() => {
            ...
            const updateIndex = that.compareIndex(
              type,
              homepage[type],
              homepage[`${type}Id`]
            );
  ...
  handleOk = type => {
    ...
    if (flag && shortflag) {
      ...
      const payload = id
        ? { type, id, data: { ..._data } }
        : { type, _params: { index: homepage[type].length + 1, ..._data } };
      dispatch({
        type: _type,
        payload: {
          ...payload
        ...
        const updateIndex = this.compareIndex(
          type,
          homepage[type],
          homepage[`${type}Id`]
        );
  ...
  parseLabel = (t, r, selectlist, label) => {
    ...
    if (label) {
      return selectlist[t].label
        ? t === "goods"
          ? r[selectlist[t].label].goods_name
          : r[t].name
        : goods_type_label;
    }
    return (
  ...
  verifyModule = key => {
    const { homepage } = this.state;
    return homepage._module.filter(
      item => item.active && item["module"] === key
    ).length;
  };
  ...
  render() {
    ...
    if (config && config.subscription_switch === false)
      _moudelConfig = _moudelConfig.filter(
        ({ module }) => module !== "periodic"
      );
            ...
            <span
              onClick={() =>
                this.delModal(
                  "homebanner",
                  r.url,
                  this.parseLabel(r.jump_type, r, selectlist, "delete")
                )
              }
            >
              <a>删除</a>
          ...
          <Row gutter={16} type="flex">
            ...
            >
              <Card
                ...
                title="首页轮播图设置"
                extra={
                  homepage.homebanner.length <= 4 && (
                    <Button
                      type="primary"
                      size="small"
                      onClick={() => this.openModal({}, "banner")}
                    >
                      新增
                    </Button>
                  )
              ...
              >
                <TableDrag
                  dataSource={homepage.homebanner}
                  columns={[...bannercolumn, ...jumpColumn, ...actionColumn_b]}
                ...
                >
                  <Spin spinning={Boolean(Loading)}>
                    <Form className={styles.editform}>
                      <FormItem
                        label="轮播图片"
                        help="支持jpg、jpeg、png格式，大小不超过2MB，建议尺寸为750*480"
                      >
                        <UploadImage
                          limit={1}
                          ...
                          onChange={(e, list) =>
                            this.handleUpload(e, "image", list)
                          }
                      ...
                      <FormItem label="跳转类型">
                        <Select
                          value={modalData.jump_type}
                          onChange={e => this.handlelevel("jump_type", e)}
                        >
                      ...
                      <FormItem label="跳转内容">
                        <SelectSearch
                          datalist={
                            (selectlist[modalData.jump_type] &&
                              selectlist[modalData.jump_type].listdata) ||
                            goodlist
                          }
                          value={modalData.link}
                          dispatchType={
                            selectlist[modalData.jump_type] &&
                            selectlist[modalData.jump_type].api
                          }
              ...
              <Card
                ...
                title="快速入口设置"
                extra={
                  homepage.shortcut.length <= 7 && (
                    <Button
                      type="primary"
                      size="small"
                      onClick={() => this.openModal({}, "enter")}
                    >
                      新增
                    </Button>
                  )
              ...
              >
                <TableDrag
                  dataSource={homepage.shortcut}
                  columns={[...entercolumn, ...jumpColumn, ...actionColumn_e]}
                ...
                >
                  <Spin spinning={Boolean(Loading)}>
                    <Form className={styles.editform}>
                      <FormItem
                        label="快速入口图片"
                        help="支持jpg、jpeg、png格式，大小不超过2MB，建议尺寸为90*90"
                      >
                        <UploadImage
                          limit={1}
                          fileList={modalData.image ? [modalData.image] : []}
                      ...
                      <FormItem label="快速入口名称">
                        <Input
                          ...
                          onChange={e =>
                            this.handlelevel("label", e.target.value)
                          }
                        />
                      </FormItem>
                      <FormItem label="跳转类型">
              ...
              <Card className={styles.main} title="首页模块设置">
                <Table
                  dataSource={_moudelConfig}
                  size="small"
                  columns={indexpartColums}
                  pagination={false}
                />
              </Card>
            </Col>
            ...
            >
              <Card title="首页预览">
                ...
                <div className={styles.onineshow}>
                  ...
                  {(this.verifyModule("grouping") && (
                    <img src={groupbuy} width="100%" />
                  )) ||
                    null}
                  {(this.verifyModule("periodic") && (
                    <img src={delivery} width="100%" />
                  )) ||
                    null}
                  {(this.verifyModule("recommendation") && (
                    <img src={recommend} width="100%" />
                  )) ||
                    null}
                  {(this.verifyModule("credit") && (
                    <img src={integrate} width="100%" />
                  )) ||
                    null}
                </div>
                <div className={styles.onineshow} style={{ height: "auto" }}>
