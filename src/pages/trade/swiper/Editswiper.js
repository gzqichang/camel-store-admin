...
class Editswiper extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      ...
      payload: { ..._data, model_type: "ord" }
    }).then(res => {
      this.setState({ goodslist: res });
    });
    dispatch({
      type: "goods/fetchGoods",
      payload: { ..._data, model_type: "sub" }
    }).then(res => {
      this.setState({ subgoodslist: res });
    });
    if (id) {
      dispatch({
    ...
    let flag = this.validatingForm({
      name:
        swiperdata.goods.model_type === "ord"
          ? swiperdata.goods_name
          : swiperdata.subgood_name,
      ...
      <PageHeaderWrapper>
        <Spin spinning={Boolean(Upload || swiperLoading)} tip="操作中">
          <Card
            ...
            title={id ? "编辑首页海报" : "新建首页海报"}
          >
            <Form className={styles.editform}>
              <FormItem wrapperCol={{ offset: 3 }}>
                <Radio.Group
                  buttonStyle="solid"
                  value={swiperdata.goods.model_type}
                  onChange={e =>
                    this.handleswiper(e.target.value, "goods", "model_type")
                  }
                >
                  <Radio.Button value="ord">商品</Radio.Button>
                  {config.store_type === "camel" && (
                    <Radio.Button value="sub">订阅商品</Radio.Button>
                  )}
                </Radio.Group>
              </FormItem>
              <FormItem label="商品名">
              ...
              <FormItem label="排序">
                <InputNumber
                  min={0}
                  onChange={e => this.handleswiper(e, "index")}
              ...
              <FormItem label="是否启用">
                <Switch
                  checked={swiperdata.is_active}
                  onChange={e => this.handleswiper(e, "is_active")}
                />
              </FormItem>
              <Row>
