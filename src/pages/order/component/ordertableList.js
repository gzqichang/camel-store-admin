  ...
  render() {
    ...
    const usualcolumns = [
        ...
        render: t => (
          <Fragment>
            {t.map(item => {
              return (
                <div>
                  {item.goods_name + "-" + item.gtype_name + "*" + item.num}
                </div>
              );
            })}
          </Fragment>
      ...
      },
      {
        ...
        render: t => (
          <span>
            {goodtype === "sub" ? substatus_group[t] : status_group[t]}
          </span>
        )
      }
    ];
              ...
              {r.status === statusText[goodtype] ? (
                moment().add(1, "days") >= moment(t) ? (
                  <Tooltip
                    placement="top"
                    title={
                      moment().subtract(1, "days") < moment(t)
                        ? "订单即将到期，请及时处理"
                        : "订单已到期，请及时处理"
                    }
                    arrowPointAtCenter
          ...
          this.setState({ loadingQuery: true }, () => {
            this.props
              ...
              .then(res => {
                ...
                const rowSelection_ = this.state.rowSelection.filter(
                  ({ id }) => !ids.includes(id)
                );
    ...
    return (
      <Fragment>
            ...
            return (
              <Fragment>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!expressCompany) return message.warn("请选择导出格式");
        ...
        >
          <Form>
            <FormItem label="订单号">{detailform.order_sn}</FormItem>
            <FormItem label="用户名">
              <Link
                style={{ color: "rgba(0, 0, 0, 0.65)" }}
                to={{
                  pathname: `/user/userlist/edituser`,
                  query: { id: detailform.user_info && detailform.user_info.id }
                }}
              >
                <img
                  src={detailform.user_info && detailform.user_info.avatar_url}
                  width="30"
                  height="30"
                  alt=""
                />
            ...
            <FormItem label="订单金额">￥ {detailform.order_amount}</FormItem>
            <FormItem label="会员钱包扣款额">
              {Number(detailform.wallet_pay) === 0
                ? "--"
                : `￥ ${detailform.wallet_pay}`}
            </FormItem>
            <FormItem label="微信支付金额">
              {Number(detailform.real_amount) === 0
                ? "--"
                : `￥ ${detailform.real_amount}`}
            </FormItem>
            ...
            <FormItem label="支付时间">
              {(detailform.pay_time &&
                moment(detailform.pay_time).format("YYYY-MM-DD kk:mm:ss")) ||
                "--"}
            </FormItem>
            <FormItem label="订单状态">
