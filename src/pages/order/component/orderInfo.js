...
class orderInfo extends Component {
  ...
  componentDidUpdate(preprops, prestate) {
    ...
    if (
      singerform &&
      singerform.url &&
      this.state.singerform.url !== singerform.url
    ) {
    ...
    if (
      ...
      ordertype !== "repl" &&
      formdata.order_sn &&
      this.state.order_sn !== formdata.order_sn
    ) {
    ...
    if (
      ordertype === "pt" &&
      formdata.ptgroup_no &&
      this.state.order_sn !== formdata.ptgroup_no
    ) {
  ...
  orderhandle = key => {
    ...
    if (key === 0) {
      this.setState({
        singerform: formdata.delivery_address,
        edit_order: false
      });
    } else {
          ...
          <Fragment>
            <Card className={styles.main} title="订单信息">
              <Form className={styles.editform}>
                ...
                <FormItem label="微信用户">
                  <Link
                    ...
                    to={{
                      ...
                      query: { id: formdata.user_info && formdata.user_info.id }
                    }}
                  >
                    <img
                      src={formdata.user_info && formdata.user_info.avatar_url}
                      width="30"
                      height="30"
                      alt=""
                    />
                    <span style={{ marginLeft: 10 }}>
                      {formdata.user_info && formdata.user_info.nickname}
                    </span>
                  </Link>
                </FormItem>
                <FormItem label="下单时间">
                  <span>
                    {formdata.add_time &&
                      moment(formdata.add_time).format("YYYY-MM-DD kk:mm:ss")}
                  </span>
                </FormItem>
                <FormItem label="支付时间">
                  <span>
                ...
                <FormItem label="下单门店">
                  <span>{formdata.shop_info && formdata.shop_info.name}</span>
                </FormItem>
                <FormItem label="配送门店">
                  <span>
                    {(formdata.entrust_shop_info &&
                      formdata.entrust_shop_info.name) ||
                ...
                {formdata.status === "paying" ? null : ordertype === "repl" ? (
                  <FormItem label="积分支付">
              ...
              <Spin tip="操作中" spinning={onLoading}>
                <Card
                  ...
                  extra={
                    edit_order ? (
                      <Fragment>
                        <Button onClick={() => this.orderhandle(0)}>
                          取消
                        </Button>
                        <Button
                          type="primary"
