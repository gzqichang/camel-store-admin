...
class goodDetail extends Component {
  ...
  componentDidMount() {
    ...
    if (!id && !is_template) {
      //创建商品且不是模板页时，获取该商品类型model_type的模板
      dispatch({
    ...
    if (id || e) {
      dispatch({
        ...
        if (res) {
           ...
           */
          if (model_type === "sub") {
            res.sub_goods.delivery_setup &&
            res.sub_goods.delivery_setup === "date"
              ? (delivery_data[res.sub_goods.date_setup] =
                  res.sub_goods.delivery_data_set)
              : null;
          }
          ...
          if (
            id &&
            (res.groupbuy || res.groupbuy_info) &&
            model_type !== "replace" &&
            !is_template
          ) {
            this.checkPtgood(id);
  ...
  */
    ...
    if (subkey) {
      ...
      if (key === "groupbuy_info") {
        ...
        if (subkey === "ladder_list") {
          ...
          //删除等级时，规格删除一个等级价格
          if (len > nowlen) {
            gooddata[textSwitch[model_type].key].gtypes &&
              gooddata[textSwitch[model_type].key].gtypes.map(item => {
                item[subkey] = item[subkey].slice(
                  0,
                  gooddata[key][subkey].length
                );
              });
          }
      ...
      if (key === "groupbuy" && e) {
        !gooddata.groupbuy_info
          ? (gooddata.groupbuy_info = { mode: "people" })
          : null;
      }
    }
    onChange(gooddata);
  ...
  handleSubmit = () => {
    ...
    let _gooddata = {
      ...
      groupbuy_info: is_build
        ...
        : gooddata.groupbuy
          ? { ...gooddata.groupbuy_info }
          : null //商品正在拼团可以取消拼团设置，不可更改阶梯数据
    };
    //订阅商品配送日期设置的处理
    if (model_type === "sub") {
      if (_gooddata.sub_goods.delivery_setup === "date") {
        _gooddata.sub_goods.delivery_data_set =
          delivery_data[gooddata.sub_goods.date_setup];
      } else {
    ...
    if (gooddata.fictitious && flag) {
      ...
      _gooddata[textSwitch[model_type].key] = {
        ...gooddata[textSwitch[model_type].key],
        estimate_time: null
      };
      if (
        (!gooddata.attach || gooddata.attach.length === 0) &&
        (config && config.attach_switch)
      ) {
        Modal.confirm({
          title: "确认操作",
          content:
            "虚拟商品不要求购买用户提供收货信息。是否需要设置自定义字段收集用户相关信息",
          okText: "去设置",
          cancelText: "不，直接创建商品",
          centered: true,
          onOk() {
            that.setState({ request: true });
          },
          onCancel() {
            that.verifysubmit(_gooddata);
          }
        });
      } else {
        is_template
          ? this.verifyTemsubmit(_gooddata)
          : this.verifysubmit(_gooddata);
      }
    } else {
      is_template
        ? flag && this.verifyTemsubmit(_gooddata)
        : flag && this.verifysubmit(_gooddata);
    }
  };
  //验证通过商品提交
  ...
  render() {
    ...
    const goodgroups = listdata.map(item => (
    ...
    return (
      <PageHeaderWrapper>
        <Card
          ...
          title={
            id
              ? `编辑${textSwitch[model_type].label}商品信息`
              : `新增${textSwitch[model_type].label}商品信息`
          }
          extra={[
            ...
            <Button
              ...
              onClick={() =>
                request &&
                this.setState({ request: false }, () => this.handleSubmit())
              }
            >
              {id == null ? "创建" : "保存"}
            </Button>
        ...
        >
          <Spin spinning={Boolean(GoodsLoading || upLoading)} tip="正在操作中">
            <Tabs defaultActiveKey="1" type="card">
                ...
                <Form>
                  ...
                  {model_type !== "replace" && (
                    <FormItem label="商品类别">
                      <CategorySelect
                        value={
                          (gooddata.category && gooddata.category.name) ||
                          gooddata.category
                        }
                  ...
                  {model_type !== "replace" && (
                    <FormItem label="接力拼团商品" required={false}>
                      ...
                      {is_build ? (
                        <a>
                          该商品仍然有人在拼团，可以关闭拼团功能，不可更改拼团数据设置
                        </a>
                      ) : null}
                    </FormItem>
                  )}
                    ...
                    <FormItem label="订阅持续时间">
                      ...
                      >
                        ...
                        <RangePicker
                          ...
                          disabled={
                            !createTem ||
                            !(gooddata.sub_goods.duration === "time_frame")
                          }
                          value={
                            gooddata.sub_goods.duration_start
                              ? [
                                  moment(gooddata.sub_goods.duration_start),
                                  moment(gooddata.sub_goods.duration_end)
                                ]
                              : null
                          }
                ...
                >
                  <Form>
                    <GroupBuy
                      ...
                      onChange={(e, label) =>
                        this.chnangegooddata(e, "groupbuy_info", label)
                      }
                    />
                  </Form>
                </TabPane>
              ...
              {this.props.config &&
                this.props.config.attach_switch && (
              ...
              <TabPane
                tab="配送设置"
                disabled={model_type !== "sub" && gooddata.fictitious}
                key="2"
                forceRender={true}
              >
                <Form>
                    ...
                    <Fragment>
                      ...
                      <FormItem label="配送日期设置">
                        <RadioGroup
                          ...
                          disabled={
                            !createTem ||
                            !(gooddata.sub_goods.delivery_setup === "date")
                          }
                        >
                        ...
                        {gooddata.sub_goods.delivery_setup === "date" &&
                        gooddata.sub_goods.date_setup === "specific" ? (
                          <DeliveryData
                            specific={delivery_data.specific || []}
                            limit={
                              gooddata.sub_goods.duration === "time_frame"
                                ? [
                                    gooddata.sub_goods.duration_start,
                                    gooddata.sub_goods.duration_end
                                  ]
                                : null
                            }
                            onChange={e =>
                        ...
                        <div
                          style={
                            gooddata.sub_goods.delivery_setup === "date" &&
                            gooddata.sub_goods.date_setup === "weekly"
                              ? {}
                              : { display: "none" }
                          }
                        >
                      ...
                      <FormItem label="固定间隔/天">
                        <InputNumber
                          disabled={
                            !createTem ||
                            !(gooddata.sub_goods.delivery_setup === "interval")
                          }
                          style={{ width: 250 }}
                          placeholder="请输入两次配送的时间间距"
                          value={gooddata.sub_goods.interval}
                          onChange={e => {
                            this.chnangegooddata(e, "sub_goods", "interval");
                          }}
                        />
                      </FormItem>
                      <FormItem label="配送日的配送时间" required={false}>
                        ...
                        <TimePicker
                          format="HH:mm:ss"
                          value={
                            gooddata.sub_goods.send_end
                              ? moment(gooddata.sub_goods.send_end, "HH:mm:ss")
                              : null
                          }
                          onChange={(e, str) => {
                            this.chnangegooddata(str, "sub_goods", "send_end");
                          }}
                        />
                      </FormItem>
                    </Fragment>
                  ...
                  {!gooddata.fictitious && (
                    <Fragment>
                      <FormItem label="配送方式">
                        ...
                        >
                          <Checkbox
                            value="own"
                            disabled={
                              gooddata.delivery_method &&
                              gooddata.delivery_method.includes("express")
                            }
                          >
                            自配送
                          </Checkbox>
                          <Checkbox
                            ...
                            disabled={
                              gooddata.delivery_method &&
                              gooddata.delivery_method.includes("own")
                            }
                          >
                            快递物流
                          </Checkbox>
                      ...
                      <FormItem
                        label="运费设置"
                        style={
                          model_type === "replace" ? { display: "none" } : {}
                        }
                      >
                        <RadioGroup
                          onChange={e => {
                          ...
                          value={
                            gooddata.postage_setup ||
                            (model_type === "replace" && "free")
                          }
              ...
              {id && (
                <TabPane tab="小程序" key="6" forceRender={true}>
                  <FormItem
                    label="页面路径"
                    help={`/pages/util/index?pid=${id}`}
                    required={false}
                  >
                    <Button
                      size="small"
                      onClick={() => copy(`/pages/util/index?pid=${id}`)}
                    >
                      复制
                    </Button>
                  </FormItem>
