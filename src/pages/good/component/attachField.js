...
import { Button, Select, Input,Divider,Switch, message, InputNumber, Row, Col, DatePicker, Radio, Checkbox, Modal, Form } from 'antd';
...
const FormItem = props => <Form.Item required {...formItemLayout} style={{marginBottom:0, textAlign:'left'}} {...props}/>
const FormItemS = props => <Form.Item labelCol={ {span: 5} } wrapperCol={ { span: 16 }} required {...props}/>
...
class attachField extends Component {
  ...
  onChangeTag = (e) => {
    ...
    let array = modalform.option || [], _array = []
    let _index = null, itemvalue = e || select_value
  ...
  handleDrag = (e) => {
    ...
    let { onChange, attach } = this.props
    const _attach = update(attach, {$splice: [[dragIndex, 1], [hoverIndex, 0, dragRow]]})
    onChange(_attach)
  }
  ...
  conversionObject() {
    ...
    attach && attach.map((item,index) => {item.key = index+1;item.index = index+1})
    return {
      attach,
      disabled,
    };
      ...
      render: (t,r) =>
        <FormItem label={r.label} required={r.is_required}>
          {r.attach_type === 'string' ?
            <Input maxLength={r.length === null ? Infinity : r.length} style={{width:200}} placeholder={r.help_text}/>
            : null
          }
          ...
          {r.attach_type === 'radio' ?
            <RadioGroup>
              {r.option.map(item_in => <Radio key={item_in} value={item_in}>{item_in}</Radio>)}
            </RadioGroup>
            : null
          }
          {r.attach_type === 'checkbox' ?
            <CheckboxGroup>
              {r.option.map(item_in => <Checkbox value={item_in}>{item_in}</Checkbox>)}
            </CheckboxGroup>
            : null
          }
        </FormItem>
      ...
      <Fragment>
        ...
        <Row>
          <Col style={{float:'right',marginTop:10,marginBottom:10}}>
            <Button type="primary" onClick={() => this.editdist(null)}>添加</Button>
          </Col>
        </Row>
        <TableDrag
        ...
        >
          <Form>
            <FormItemS label="字段名称">
              <Input value={modalform.label}
                     onChange={(e) => this.onChangeModal(e.target.value,'label')}
                     placeholder="建议名称不超过6个字"/>
            </FormItemS>
            <FormItemS label="类型">
              <Select value={modalform.attach_type} onChange={(e) => this.onChangeModal(e,'attach_type')}>
            ...
            <FormItemS label="是否必填">
              <Switch checked={modalform.is_required} onChange={(e) => this.onChangeModal(e,'is_required')}/>
            </FormItemS>
            <FormItemS label="备注文字" required={false}>
              <Input value={modalform.help_text}
            ...
            { modalform.attach_type === 'number' ?
              <Fragment>
                <FormItemS label="最小值" required={false}>
                  <InputNumber value={modalform.min_value}
                               onChange={(e) => this.onChangeModal(e,'min_value')}/>
                </FormItemS>
                <FormItemS label="最大值" required={false}>
                  <InputNumber value={modalform.max_value}
                               onChange={(e) => this.onChangeModal(e,'max_value')}/>
                </FormItemS>
              </Fragment>
              : null
            }
            {modalform.attach_type === 'radio' || modalform.attach_type === 'checkbox' ?
              <FormItemS label="值">
                <TagSelect children={modalform.option} onClose={(e) => this.onChangeTag(e)}/>
                <Input placeholder="请输入选项名称"
                       value={select_value}
                       onChange={(e) => this.setState({ select_value: e.target.value}) }/>
                <Button size="small" type="primary" onClick={() => this.onChangeTag()}>添加</Button>
              </FormItemS>
              : null
            }
          </Form>
