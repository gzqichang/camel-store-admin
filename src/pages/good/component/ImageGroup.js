...
const FormItem = props => (<Form.Item className={styles.imageItem} required {...formItemLayout} {...props}/>);
...
class ImageGroup extends Component {
  ...
  handleSelect = () => {
    ...
    if(key === 'banner'){
      const select_List = _selectList.map(item => {return { image:{ ...item, index:undefined, key:undefined } } })
  ...
  handleUpload = (res,key,list,isdrag) => {
    ...
    if(key === 'banner'){
      if(isdrag && isdrag === 'drag'){
        ...
        arrlist = update(bannergroup, {$splice: [[dragIndex, 1], [hoverIndex, 0, dragRow]]})
      }
      else{
        arrlist = del ? imageTransformReverse(bannergroup,list) : [...bannergroup, { image: {...res} }]
      }
    }
    else if(key === 'detail'){
      if(isdrag && isdrag === 'drag'){
        const { dragIndex, hoverIndex, dragRow } = res
        arrlist = update(detailgroup, {$splice: [[dragIndex, 1], [hoverIndex, 0, dragRow]]})
      }
      else {
        arrlist = del ? imageTransformReverse(detailgroup, list) : [...detailgroup, { image: { ...res } }]
      }
    }
    else{//key=== 'poster,image,video'
  ...
  conversionObject() {
    const { video, goodimg, poster, detailgroup, bannergroup, disabled } = this.props;
  ...
  render() {
    const { video, goodimg, poster, detailgroup, bannergroup, disabled } = this.conversionObject()
    const { visible, key, limit } = this.state
    const wh = window.screen.height;

    return(
      <Fragment>
        <FormItem label="商品介绍视频" required={false} help="支持MP4，建议视频大小10M以内。"
          ...
          <UploadImage
            ...
            handleSource={() => this.openModal(1 - video.length,'video',video.length < 1)}
            restprops={{ text: '上传视频', listType:'text', accept:'.mp4', openFileDialogOnClick: false }}
          />
        </FormItem>
        <FormItem label="朋友圈分享海报" help='支持PNG, JPG，建议像素宽度为584，高度为910。'>
          <UploadImage
            areakey={'poster'}
            disabled={disabled}
            ...
            limit={1}
            fileList={poster}
            handleSource={() => this.openModal(1 - poster.length,'poster',poster.length < 1)}
            onChange={(e,list,isdrag) => this.handleUpload(e,'poster',list,isdrag)}
        ...
        {/*<FormItem label="封面图" help='支持PNG, JPG，建议像素宽度为670，高度为306。'>*/}
          ...
          {/*/>*/}
        {/*</FormItem>*/}
        <FormItem label="详情图" help='支持PNG, JPG, 建议像素宽度为750。(请尽量控制图片大小和张数，以免客户浏览的时候加载太慢)'>
          <UploadImage
            isdrag={true}
            areakey={'detail'}
            disabled={disabled}
            ...
            onChange={(e,list,isdrag) => this.handleUpload(e,'detail',list,isdrag)}
            restprops={{ openFileDialogOnClick: false }}
          />
        </FormItem>
        <FormItem label="轮播图" help='支持PNG, JPG, 建议像素为750 * 720。(图片最多为5张)'>
          <UploadImage
            isdrag={true}
            ...
            handleSource={() => this.openModal(5 - bannergroup.length,'banner',bannergroup.length < 5)}
            onChange={(e,list,isdrag) => this.handleUpload(e,'banner',list,isdrag)}
            restprops={{ openFileDialogOnClick: false }}
          />
        </FormItem>
