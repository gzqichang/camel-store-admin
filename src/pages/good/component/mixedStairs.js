...
class mixedStairs extends Component {
  ...
  adddist = () => {
    ...
    if (edit) {
      ...
      if (!flag) {
        ...
        this.setState({
          postage_typemax: postage_typemax + 1,
          stairs: [...stairs]
        });
      }
    } else {
  ...
  render() {
    ...
    return (
      <Fragment>
        <RadioGroup
          disabled={Boolean(goodtype || disabled)}
        ...
        <List
          ...
          renderItem={item => (
            <List.Item
              actions={[
                <a
                  onClick={() => this.disthandle(item, "", item.index)}
                  style={edit ? { color: "red" } : { display: "none" }}
                >
                  删除
                </a>
              ]}
            >
              ...
              {edit ? (
                <InputNumberItem
                  ...
                  min={
                    item.index - 1 > 0
                      ? Number(_stairs[item.index - 2].num) + 1
                      : 2
                  }
          ...
          footer={
            <Fragment>
              <Button size="small" onClick={this.adddist} disabled={disabled}>
                {edit ? "添加" : "编辑"}
              </Button>
              {edit ? (
                <Button
