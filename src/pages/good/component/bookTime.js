...
class bookTime extends Component {
  ...
  addlist = () => {
    ...
    let { postage_typemax } = this.state;
    let flag = true;
    estimate_time && estimate_time.length > 0
      ? estimate_time.map(list => {
          Object.keys(list).map(tt => {
            const arr = !(Array.isArray(list[tt])
              ? list[tt].length > 1
              : list[tt] !== "");
            flag = !arr && flag;
          });
      ...
      if (len > 0 && estimate_time[len - 1].add[1] === "23:59") {
        ...
        estimate_time.length === 0
          ? estimate_time.push({
              ...
              send: [],
              sendtype: "",
              key: postage_typemax + 1
            })
          : estimate_time.push({
              add: [estimate_time[len - 1].add[1], "23:59"],
              send: [],
              sendtype: "",
              key: postage_typemax + 1
            });
  ...
  ontimeChange = (str, keyname, _index, key) => {
    ...
    if (keyname === null) {
      ...
      edit && estimate_time.length > itemindex + 1
        ? (estimate_time[itemindex + 1].add[0] = str)
        : null;
    }
    if (!disabled) {
      onChange(estimate_time);
    ...
    const column = [
      {
        ...
        render: (t, r) => (
          <Fragment>
            <Time_Picker
              value={r.add[0] ? moment(r.add[0], format) : null}
              disabled={true}
              onChange={(time, timeString) =>
                this.ontimeChange(timeString, "add", 0, r.key)
              }
            />
            <span style={{ marginLeft: 5, marginRight: 5 }}>-</span>
            <Time_Picker
              value={r.add[1] ? moment(r.add[1], format) : null}
              disabled={disabled}
              disabledHours={() =>
                this.disabledHours(moment(r.add[0]).format("HH"))
              }
              disabledMinutes={e => this.disabledMinutes(e, r.add[0])}
              onChange={(time, timeString) =>
                this.ontimeChange(timeString, "add", 1, r.key)
              }
            />
          </Fragment>
        )
      },
      {
        ...
        render: (t, r) => (
          <Fragment>
            ...
            <Time_Picker
              value={r.send[0] ? moment(r.send[0], format) : null}
              disabled={disabled}
              onChange={(time, timeString) =>
                this.ontimeChange(timeString, "send", 0, r.key)
              }
            />
            <span style={{ marginLeft: 5, marginRight: 5 }}>-</span>
            <Time_Picker
              value={r.send[1] ? moment(r.send[1], format) : null}
              disabled={disabled}
              disabledHours={() =>
                this.disabledHours(r.send[0] ? r.send[0].split(":")[0] : 0)
              }
              disabledMinutes={e =>
                this.disabledMinutes(e, r.send[0] || "00:00")
              }
              onChange={(time, timeString) =>
                this.ontimeChange(timeString, "send", 1, r.key)
              }
            />
          </Fragment>
        )
      },
      {
        ...
        render: (t, r) => (
          <Fragment>
            <a
              onClick={() => this.ontimeChange("", null, null, r.key)}
              style={{ color: "red" }}
            >
              删除
            </a>
