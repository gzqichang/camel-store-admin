...
class deliveryData extends Component {
  ...
  disabledDate = current => {
    ...
    if (limit) {
      if (day < moment(limit[0])) {
        //以后开始
        return (
          current && (current > moment(limit[1]) || current < moment(limit[0]))
        );
      } else {
        //已经开始
        return (
          current &&
          (current < moment().endOf("day") || current > moment(limit[1]))
        );
      }
    }
    ...
    return (
      <Fragment>
        <Row>
          ...
          <Col
            style={
              !specific || specific.length === 0 ? { marginTop: "40px" } : {}
            }
          >
            <DatePicker
              disabled={disabled}
                ...
                return (
                  <Row>
                    <Col style={{ textAlign: "right" }}>
                      <Button
                        size="small"
                        onClick={() => this.setState({ openVisible: false })}
                      >
                        关闭
                      </Button>
                      <Button
