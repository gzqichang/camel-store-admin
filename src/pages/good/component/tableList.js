...
class tableList extends Component {
  ...
  deleteModal = (key,name,id) => {
    ...
    if(edit){
      ...
      isTemplate ? message.error("你没有进行该操作的权限") : this.deleteMothod(key,name,id)
    }
  }
  ...
  //处理传值
  conversionObject() {
    const { loading, dataSource, pagination, path, isTemplate, replacegood = false } = this.props;
    return {
      loading,
      dataSource,
      pagination,
  ...
  render(){
    const { loading, dataSource, pagination, path, isTemplate, replacegood } = this.conversionObject();
    ...
    const startcolumns = [
      {
        ...
        dataIndex: 'category',
        key: 'category',
        render: (t,r) => replacegood ? '积分商品' : r.category && r.category.name
      }, {
        title: '封面图',
        dataIndex: 'banner',
        key: 'banner',
        render: (t,r) => (
          t[0] && t[0].image && <img src={t[0].image.file} width={120}/> || null
        )
      },];
    const endcolumns = [
      {
        ...
        render: (text, record) => (
          <Fragment>
           <span onClick={() => this.deleteModal(record.key,record.name,record.id)}>
             <a>删除</a>
           </span>
    ...
    return(
      <Fragment>
        <Table
          ...
          columns={isTemplate ? [...startcolumns,...endcolumns] : [...startcolumns,...statuscolumns,...endcolumns]}
