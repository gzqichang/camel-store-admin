...
class Categorylist extends Component {
  ...
  statusChange = (is_active,id,name) => {
    ...
    Modal.confirm({
      title: '确认操作 ',
      content: `确认${is_active ? `禁用` : `启用`} ${name} 类别${is_active ? `，下架此分类所有商品` : ``}`,
      centered: true,
      okText: '确认',
      okType: 'danger',
      cancelText: '取消 ',
  ...
  render(){
    ...
    const columns = [
      {
        ...
        key: 'is_active',
        render: (t,r) => (
          <span onClick={() => this.statusChange(t,r.id,r.name)} style={{cursor:'pointer'}}>
          {t ?
        ...
        render: (text, record) => (
          <Fragment>
           <span onClick={() => this.deleteModal(record.name,record.id)}>
            ...
            <span>
             <Link to={{pathname:`/good/categorylist/editpage`,query: { id:record.id} }}>编辑</Link>
           </span>
          </Fragment>
        ),
      }];
    ...
    return(
      ...
      <Card className={styles.main}>
        <Link to={{pathname:`/good/categorylist/editpage`,query: {id:null} }}>
          <Button type='primary' style={{ marginBottom: 20 }}>新增类别</Button>
        </Link>
        <Table
