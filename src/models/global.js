...
export default {
  ...
  effects: {
    *fetchAppQRCode(_, { call, put }) {
      const { public_code = null, preview_code = null } = yield call(queryWxappQRCode) || {};
      yield put({
        type: 'save',
        payload: {
          appQRCode: {
    ...
    *fetchFeedback({ payload }, { select, call, put }) {
      ...
      if (response) {
        ...
        yield put({
          type: 'save',
          payload: {
            noticefeedback: Array.isArray(response.results) ? response.results : [],
        ...
        if (list) {
          ...
          yield put({
            ...
            payload: {
              ...
              sendorderlist:
                response.results.length > 10 ? response.results.slice(0, 10) : response.results,
