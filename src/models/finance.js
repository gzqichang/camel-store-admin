import { getwithdrawalList, getwithdrawalData, withdrawalDataStatus,
  getaccountlogList,
  getRecgargelist,
...
export default {
  ...
  effects: {
    ...
    *getwithdrawalList({ payload }, { call, put }) {
      ...
      const { page } = payload
      response.results.map((item,index) => item.key = (page-1)*10 + index+1)
      yield put({
        type: 'save',
        payload: {
          withdrawallist: Array.isArray(response.results) ? response.results : [],
    ...
    *fetchaccountlogList({ payload }, { call, put }) {
      ...
      if(response){
        ...
        yield put({
          type: 'save',
          payload: {
            accountlogList: Array.isArray(response.results) ? response.results : [],
