...
export default {
  ...
  effects: {
    * fetchExtensionList(_, {call, put}) {
      ...
      if (ext && ext.results) {
        const rs = ext.results.map((item) => {
          return {
            isBrought: item.receipt_info !== null
              && item.receipt_info.is_brought === true,
            expired: item.receipt_info === null
              ? null
              : item.receipt_info.expire_date,
            ...item,
          }
        });
        yield put({
    ...
    * createOrder({payload: {id}}, {call, put}) {
    ...
    * cancelOrder({payload: {order_sn}}, {call, put}) {
      yield put({
        type: 'save',
        payload: {
          isCancel: false,
