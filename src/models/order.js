...
export default {
  ...
  effects: {
    ...
    *fetchOrder({ payload }, { call, put }) {
      ...
      if(model_type === 'ord'){
        ...
        yield put({
          type: 'save',
          payload: {
            suborderlist: Array.isArray(response.results) ? response.results : [],
        ...
        yield put({
          type: 'save',
          payload: {
            replorderlist: Array.isArray(response.results) ? response.results : [],
        ...
        yield put({
          type: 'save',
          payload: {
            qrpayorderlist: Array.isArray(response.results) ? response.results : [],
    ...
    *fetchExportList({ payload }, { call, put }) {
      ...
      const _pay = {
        ...payload,
        send_type:status === 'has paid' ? 'sending' : (status === 'done' ? 'over' : status),
      };
      ...
      let orders = [['序号','订单号', '商品','规格','数量', '价格（元）','总价（元）', '下单时间','配送方式','订单状态']]; // 总的列表
      let count = 1; // 标记 “序号”
      let page = 1; // 总页数
      if(response){
       ...
       rep.map(
         (item) => {
           orders.push([
             ...
             ( Number(item.goods_backup.price)*Number(item.goods_backup.num)).toFixed(2)|| '',
             item.add_time && moment(item.add_time).format('YYYY-MM-DD kk:mm:ss'),
             item.goods_backup.delivery_method && deliverymethod[item.goods_backup.delivery_method] || '',
             item.zh_send_type || ''
           ]);
           count++;
         },
    ...
    *fetchqrExportList({ payload }, { call, put }) {
      const response = yield call(getOrderList, payload );
      let orders = [['序号','订单号', '订单金额（元）','会员钱包金额（元）','微信支付金额（元）', '支付时间','微信用户','订单状态']]; // 总的列表
      let count = 1; // 标记 “序号”
      let page = 1; // 总页数
      const status_group = {
        paying: '待付款',
        ...
        rep.map(
          (item) => {
            orders.push([
              ...
              item.wallet_pay || '',
              item.real_amount || '',
              item.pay_time && moment(item.pay_time).format('YYYY-MM-DD kk:mm:ss'),
              item.user_name,
    ...
    *getExpresslist({ payload }, { call, put }) {
      ...
      if(response){
        yield put({
          type: 'save',
          payload: {
            expresslist: Array.isArray(response.results) ? response.results : [],
