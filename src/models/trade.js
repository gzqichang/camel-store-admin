import {
  getExpandData, setExpandData, getStorePoster, setStorePoster,queryExcel,
  getTradelist, getTradeData, updateTrade, createTrade, deleteTrade,exportExcel,
} from '@/services/trade';
  ...
  effects: {
    ...
    *fetchrecharge({ payload }, { call, put }) {
      ...
      yield put({
        type: 'save',
        payload: {
          rechargetypelist: Array.isArray(response.results) ? response.results : [],
    ...
    *fetchhomepageList({ payload }, { call, put }) {
      ...
      response.results.map((item, index) => (item.key = index + 1));
      let data = {}, res = Array.isArray(response.results) ? response.results : [];
      let _key = null;
      const _list = data => data.map((item) => {
        ...
        let link = null;
        if(jump_type === 'category') {  //商品分类
          link = category && category.url
        }
        if(jump_type === 'goods') {  //商品详情
          link = goods
        }
        if(jump_type === 'goods_type') {  //商品类型
          link = goods_type
        }
        return {
          ...item,
