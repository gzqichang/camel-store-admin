  ...
  render() {
    ...
    return (
      <span
        className={
          isSelect
            ? `${styles[className]} ${styles.unmultiOpera}`
            : styles.unmultiOpera
        }
      >
        <div className={`ant-upload-list ant-upload-list-picture-card`}>
          <span className={styles.Itemcheckbox}>
            <Checkbox checked={isSelect} onChange={() => handleItem(item)} />
          </span>
          <div
              ...
              <span>
                ...
                >
                  {item.file_type === "picture" ? (
                    <img
                      src={`${item.file}?imageView2/0/w/102/h/102`}
                      style={{ width: "102px", height: "102px" }}
                      alt={item.label}
                    />
                  ) : (
                    <video
                      src={item.file}
                      poster={`${item.file}?vframe/jpg/offset/1`}
                      preload="meta"
                      style={{ width: "102px", height: "102px" }}
                    />
                  )}
              ...
              <div className={styles.title_time}>
                ...
                <Row>
                  <Col span={12} style={{ textAlign: "left" }}>
                    {moment(item.create_at).format("kk:mm")}
                  </Col>
...
class SourceImage extends Component {
  ...
  handleDelete = e => {
    ...
    let file_List = fileList
      .filter(
        item => (item.image ? item.image.url !== e.key : item.url !== e.key)
      )
      .map(item => {
        return item.image ? item.image.url : item.url;
      });
  ...
  render() {
    ...
    return (
      <Fragment>
          ...
          {type !== "video"
            ? fileList.map((item, index) => (
                <BodyRow
                  ...
                  handleDelete={
                    onChange ? item => this.handleDelete(item) : undefined
                  }
                  handleEdit={
                    onhandleEdit ? item => onhandleEdit(item) : undefined
                  }
                  handleItem={item => restprops && restprops.onSelectItem(item)}
                  className={(restprops && restprops.className) || undefined}
                  isSelect={
                    restprops &&
                    restprops.selectList &&
                    restprops.selectList.includes(item.url)
                  }
                />
              ))
            : null}
        ...
        >
          {previewImage &&
          ["mp4", "mp3"].includes(previewImage.split(".").reverse()[0]) ? (
            <video
              src={previewImage}
              style={{ maxHeight: `${wh - 360}px` }}
              controls="controls"
            />
          ) : (
...
class sourceImageTab extends Component {
  ...
  handleImageTag = () => {
    ...
    zipuploadList.length > 0 &&
      zipuploadList.map(item => {
        const size = item.size / (1024 * 1024); //mb
        if (size < 10) {
          dispatch({
  ...
  getTagItem = e => {
    this.setState({
      tagSelectvisible: true,
      itemTag: { ...e },
      Itemtaglist: e.tag ? [...e.tag] : []
    });
  };
  changeTag = () => {
    ...
    return (
      <Fragment>
          ...
          <Layout style={{ background: "#fff" }}>
            ...
            >
              ...
              <div className={styles.sliderTag}>
                <span
                  className={
                    taglist.filter(item => item.check).length === 0
                      ? styles.tagcheck
                      : null
                  }
                >
                ...
                {taglist &&
                  taglist.map(item => (
                    <span className={item.check ? styles.tagcheck : null}>
                      <Tag
                        closable
                        afterClose={() => this.handleTag(item, "delete")}
                      >
                        <span onClick={() => this.selectTag(item.id)}>
                          {item.content}
                        </span>
                      </Tag>
                    </span>
                ...
                <Popover
                  ...
                  content={
                    <Fragment>
                      ...
                      <Button
                        size="small"
                        onClick={() => this.handleTag(null, "close")}
                      >
                        取消
                      </Button>
                    </Fragment>
            ...
            <Content style={{ padding: "0 24px" }}>
                ...
                <Col>
                  ...
                  <div style={{ float: "right" }}>
                    <Button
                      ...
                      type="primary"
                      icon="upload"
                      onClick={() => this.setState({ selectModal: true })}
                    >
                      上传
                      {typeText[type]}
                    </Button>
                    {editIcon ? (
                      <Button
                        ...
                        size="small"
                        style={{ marginLeft: 10 }}
                        onClick={() => this.MorehandleDelete(null)}
                      >
                        删除
                      </Button>
                    ) : null}
              ...
              <SourceImage
                ...
                onChange={
                  editIcon
                    ? (res, list) => this.handleUpload(res, list)
                    : undefined
                }
        ...
        <Modal
          ...
          footer={[
            <Button onClick={() => this.setState({ selectModal: false })}>
              取消
            </Button>,
            <Button type="primary" onClick={() => this.handleImageTag()}>
              确定
            </Button>
          ]}
        >
          <Form className="uploadModal">
              ...
              {_tag_list &&
                _tag_list.map(item => (
                  <CheckableTag
                    ...
                    onChange={() => this.checkhandleTag(item.url)}
                  >
                    {item.content}
                  </CheckableTag>
                ))}
            ...
            <FormItem
              label="批量上传"
              help={`压缩包的格式为zip，包内${typeText[type]}支持
              ${
                type === "picture" ? "jpeg、jpg、png" : "mp4"
              }格式，大小不超10MB`}
            >
              <UploadImage
                type="video"
                ...
                onChange={(e, list) =>
                  this.handleUpload(e, list, "zipuploadList", "manualUpload")
                }
                restprops={{
                  text: `选择${typeText[type]}压缩包`,
                  listType: "text",
                  accept: ".zip",
                  showUploadList: true
                }}
            ...
            >
              <UploadImage
                ...
                fileList={uploadList || []}
                onChange={(e, list) =>
                  this.handleUpload(e, list, "uploadList", "manualUpload")
                }
                restprops={{
                  accept: type === "picture" ? ".jpg,.jpeg,.png" : ".mp4",
                  text: "选择视频"
                }}
              />
            </FormItem>
          ...
          {taglist &&
            taglist.map(item => (
              <CheckableTag
                ...
                onChange={() => this.checkhandleTag(item.url, "item")}
              >
                {item.content}
              </CheckableTag>
            ))}
